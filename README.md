...
One retained snapshot per topic is kept outside live count and age limits. A superseded retained snapshot may be removed under shared storage pressure; it never takes priority over a newer received value. Retained snapshots are inspectable and do not add plot samples or create line breaks because their publication time is unknown. Payloads over 1 MiB are omitted.

The shared storage budget is 100,000 messages and 64 MiB of estimated storage. Older history is trimmed first, so **Live max** is an upper bound rather than a reservation. A topic needing more nodes than remain in the 10,000-node tree budget is ignored; topics that fit can still be added, and existing topics continue updating. If even the latest values alone cannot fit the storage budget, collection stops with an explicit notice; the connection stays open and collected data remains inspectable.
