...
Plots use browser receipt time. Display updates are batched; every accepted arrival enters history, including retained replays and empty payloads. Neither erases earlier messages. Retained replays are excluded from plots so refreshing subscriptions does not count old values as new measurements. A pinned field becoming absent or nonnumeric interrupts its line; the plot resumes when numeric values return.
